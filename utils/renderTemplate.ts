...
function renderTemplate(src, dest, callbacks) {
  ...
  if (filename === 'tsconfig.app.json' && fs.existsSync(dest)) {
    // merge instead of overwriting
    const existing = JSON.parse(fs.readFileSync(dest, 'utf8'))
    const newSettings = JSON.parse(fs.readFileSync(src, 'utf8'))
    const settings = deepMerge(existing, newSettings)
  ...
  if (filename === '_env' && fs.existsSync(dest)) {
    // append to existing .env
    const existing = fs.readFileSync(dest, 'utf8')
    const newEnv = fs.readFileSync(src, 'utf8')
    fs.writeFileSync(dest, existing + '\n' + newEnv)
    return
  }

  if (filename === 'env.d.ts' && fs.existsSync(dest)) {
    // append to existing env.d.ts
    const existing = fs.readFileSync(dest, 'utf8')
    const newEnvDts = fs.readFileSync(src, 'utf8')
    fs.writeFileSync(dest, existing + '\n' + newEnvDts)
    return
  }
